...
from vts_api.types import class_by_event_name_list, EventTypes
...
class Listener:
    ...
    async def start_listening(self):
        ...
        async for msg in self._websocket:
            ...
            if msg.type == WSMsgType.TEXT:
                ...
                logger.debug(f"Got an error from websocket. msg: {msg}")
                break
    ...
    async def _on_message(self, data):
        if data["messageType"] == "APIError":
            ...
            logger.error(f"Server response an API Error. Data: {error_data.data}")
            ...
            if event_class:
                all_handlers = (
                    ...
                    + [
                        ...
                        if "_ANY_EVENT_" in self._handlers
                        else []
                    ][0]
                )
                ...
                for handler in all_handlers:  # Calling unskippable handlers first
                    if not handler[0]:
                        await self.loop.create_task(
                            handler[1](event_class.model_validate(data))
                        )
                ...
                for handler in all_handlers:
                    try:
                        ...
                        logger.error(
                            f"Exception caught while calling the {handler[1].__name__}. Skipping handler. Exception: {ex}"
                        )
                    else:
                        break
    ...
    def add_listener(
        ...
        if not asyncio.iscoroutinefunction(function):
            raise ValueError("You should add only coroutines to listeners.")

        if event_type not in self._handlers:
            self._handlers[event_type.value] = []

        self._handlers[event_type.value].append([skippable, function])
